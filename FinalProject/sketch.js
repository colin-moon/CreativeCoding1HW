...
var obstacles = [
  {
    x: 200, y: 350, diameter: 20, speedY:3, color: [200,0,0]
  },
  {
    x: 500, y: 350, diameter: 20, speedY:3, color: [200,0,0]
  },
  {
    x: 350, y: 200, diameter: 20, speedX:3, color: [200,0,0]
  },
  {
    x: 350, y: 500, diameter: 20, speedX: 3, color: [200,0,0]
  }
];
...
function draw() {
  ...
  for (var i = 0; i < obstacles.length; i++) { // Obstacle movement
    ...
    if (i < 2) { 
      obs.y += obs.speedY;
      if (obs.y >= 700 || obs.y <= 0) { // Vertical movement
        obs.speedY *= -1;
      }
    }
    ...
    if (i ===2 || i===3) {
      obs.x += obs.speedX;
      if (obs.x >= 700 || obs.x <= 0) { // Horizontal movement
        obs.speedX *= -1;
      }
    }
  }
